import HeadMeta from '../components/HeadMeta';
import BlogLeftPage from '../containers/Pages/BlogLeftPage'
import { TOKEN } from '../services/API';
import { fetcher } from '../utils/fetcher';
const BlogLeft = ({ articles = [], categories = [], publications = []}) => {
  return (
    <>
      <HeadMeta />
      <BlogLeftPage  articles={articles} categories={categories} publications={publications} />
    </>
  )
}

export default BlogLeft


export async function getStaticProps() {
  const token = TOKEN();
  const catsToken = 'Bearer '
  const articles = await fetcher('articolos?populate=*', token);
  const categories = await fetcher('articolos?populate=*', catsToken);
  const publications = await fetcher('pubblicazionis?populate=*', token);

  return {
      props: {
          categories,
          articles,
          publications
      }
  }
}