import HeadMeta from '../components/HeadMeta';
import SinglePortfolioPage from '../containers/Pages/SinglePortfolioPage'
import { TOKEN } from '../services/API';
import { fetcher } from '../utils/fetcher';
const PortfolioSingle = ({ articles, categories, sideBarCategories}) => {
  return (
    <>
      <HeadMeta />
      <SinglePortfolioPage categories={categories} sideBarCategories={sideBarCategories} articles={articles} />
    </>
  )
}

export default PortfolioSingle

export async function getServerSideProps() {
  const token = TOKEN();
  const catsToken = 'Bearer '
  const articles = await fetcher('articolos?populate=*', token);
  const categories = await fetcher('articolos?populate=*', catsToken);
  const sideBarCategories = await fetcher('categorias?populate=*', token);


  return {
      props: {
          categories,
          articles,
          sideBarCategories
      }
  }
}